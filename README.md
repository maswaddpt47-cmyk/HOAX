...
- **E-mail** — analyse un texte de mail collé (objet + corps) et une adresse d'expéditeur optionnelle : formulations d'urgence, demandes d'informations sensibles, salutations génériques, incohérence entre marque citée et domaine réel de l'expéditeur, mais aussi les signaux du démarchage abusif (rénovation énergétique, promesse d'avance de frais, demande de répondre avec une formule d'accord pré-rédigée, demande groupée de documents administratifs). Tout le traitement se fait dans le navigateur, rien n'est envoyé sur un serveur.
- **SMS** — analyse un texte de SMS collé et un expéditeur optionnel (numéro ou nom) : formulations d'urgence, demandes d'informations sensibles, lien raccourci ou vers une extension suspecte, numéro surtaxé (08/09), piège « répondez OUI », incohérence entre marque citée et numéro de mobile standard utilisé pour l'envoi. Module indépendant du module E-mail, avec ses propres exemples.

Les modules E-mail et SMS détectent aussi les **domaines proches mais différents** mentionnés dans un même texte (ex. `swissenerg.com` vs `swissenergie.fr`, souvent visibles dans une signature) — un signal fréquent d'usurpation d'identité d'entreprise, à vérifier via WHOIS/SIRET.
